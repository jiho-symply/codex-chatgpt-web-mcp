...
- **Windows:** Microsoft Edge → Google Chrome → 이미 설치된 Playwright Chromium
- **Linux:** Google Chrome/Chromium → 설치된 Microsoft Edge → 이미 설치된 Playwright Chromium

지원 브라우저가 없다면 Edge/Chrome/Chromium을 설치하세요. Playwright Chromium은
선택적 fallback으로 사용할 수 있습니다.
...
일반 사용자는 clone/build가 필요 없습니다. GitHub에서 `npx`로 실행할 때 npm의
`prepare`가 TypeScript build를 자동 수행합니다.
